...
const opts = {
    ...
    ySearch(searchQuery, opts, function (err, results) {
        if (err) {
            msg.channel.send(`J'ai pas de musiques avec ces nom là.. Tu t'est planté peut être ?`);
    ...
    */
    song(msg, guild, url) {
        ytdl.getInfo(url, (err, song) => {
            if (err) {
                console.log(err);
                msg.channel.send(`Je peux pas pour l'instant.. Désolé..`);
                return;
            }
            const author  = msg.author.username + '#' + msg.author.discriminator;
            console.log(song);
            ...
            msg.channel.send(
                `Mis en place de la musique ${tool.wrap(song.title.trim())} (\`${time(song.length_seconds)}\`) Demandée par ${tool.wrap(author)}`
            );
            if (guild.status != 'playing') {
                guild.playSong(msg);
            }
    ...
    for (let guildId in guilds) {
        ...
        if (guild.inactivityTimer <= 0) {
            ...
            guild.musicChannel.send(
                ':no_entry_sign: Bon, moi je me casse.');
